...
public class FormResidencyFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        view = inflater.inflate(R.layout.fragment_form_residency, container, false);
    ...
    private void handleUserInteraction() {
        btnSubmit.setOnClickListener(view -> validateResidencyForm());
    ...
    private void submitForm() {
        ...
        Map<String, Object> residencyRequest = new HashMap<>();
        residencyRequest.put("userUid", AUTH.getUid());
        residencyRequest.put("firstName", firstName);
        residencyRequest.put("middleName", middleName);
        residencyRequest.put("lastName", lastName);
        residencyRequest.put("birthdate", birthdate);
        residencyRequest.put("addressPurok", addressPurok);
        residencyRequest.put("civilStatus", civilStatus);
        residencyRequest.put("residencyStatus", residencyStatus);
        residencyRequest.put("purpose", purpose);
        residencyRequest.put("timestamp", System.currentTimeMillis());
        residencyRequest.put("status", "PENDING");
        residencyRequest.put("documentType", "RESIDENCY");

        DocumentReference refRequest =  DB.collection("requests").document();

        residencyRequest.put("uid", refRequest.getId());

        boolean userIsResident = radioResident.isChecked();
        if (!userIsResident) {
            letterOfRequest = etLetterOfRequest.getText().toString();
            residencyRequest.put("letterOfRequest", letterOfRequest);
                        ...
                        sRefCedula.putFile(uriSelectedForCedula).addOnCompleteListener(new OnCompleteListener<UploadTask.TaskSnapshot>() {
                            ...
                            public void onComplete(@NonNull Task<UploadTask.TaskSnapshot> task) {
                                if (!task.isSuccessful()) {
                                    ...
                                    refRequest.set(residencyRequest)
